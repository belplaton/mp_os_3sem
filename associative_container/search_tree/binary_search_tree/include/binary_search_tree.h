...
protected:
    ...
    public:
        ...
        virtual iterator_data* construct_iterator_data(int depth);
...
public:
    ...
    private:
        ...
        std::stack<std::pair<node*, int>> traversal_stack;
        std::stack<std::pair<node*, int>> post_order_stack;
        iterator_data* current_data;
...
protected:
    ...
    public:
        ...
        typename binary_search_tree<tkey, tvalue>::insertion_of_existent_key_attempt_strategy get_insertion_strategy() noexcept;
    
    protected:
        
        [[nodiscard]] inline virtual size_t obtain_node_size() const noexcept;
...
typename binary_search_tree<tkey, tvalue>::iterator_data*
    binary_search_tree<tkey, tvalue>::node::construct_iterator_data(int depth)
{
    iterator_data* data = new iterator_data(depth, key, value);
    return data;
}
    ...
    binary_search_tree<tkey, tvalue>* tree)
{
    ...
    std::stack<typename binary_search_tree<tkey, tvalue>::node*> nodes_to_delete;
    nodes_to_delete.push(*other);

    while (!nodes_to_delete.empty())
    {
        auto* current = nodes_to_delete.top();
        nodes_to_delete.pop();

        if (current->left_subtree)
        {
            nodes_to_delete.push(current->left_subtree);
        }
    ...
    typename binary_search_tree<tkey, tvalue>::node *subtree_root) : current_data(nullptr), traversal_stack()
{
    if (subtree_root)
    {
        ...
        current_data = subtree_root->construct_iterator_data(0);
    }
...
typename binary_search_tree<tkey, tvalue>::prefix_iterator &binary_search_tree<tkey, tvalue>::prefix_iterator::operator++()
{
    ...
    if (!traversal_stack.empty())
    {
        ...
        delete current_data;
        current_data = next_node->construct_iterator_data(next_depth);
    }
    else
    {
        delete current_data;
    ...
    typename binary_search_tree<tkey, tvalue>::node *subtree_root) : current_data(nullptr), traversal_stack()
{
    if (subtree_root)
    {
        ...
        while (current)
        {
            ...
            {
                current_data = current->construct_iterator_data(traversal_stack.size() - 1);
                break;
            }
...
typename binary_search_tree<tkey, tvalue>::infix_iterator &binary_search_tree<tkey, tvalue>::infix_iterator::operator++()
{
    ...
    if (current->right_subtree)
    {
        ...
        delete current_data;
        auto [current, depth] = traversal_stack.top();
        current_data = current->construct_iterator_data(depth);
        return *this;
    }
    ...
    while (!traversal_stack.empty())
    {
        ...
        {
            delete current_data;
            current_data = parent->construct_iterator_data(parent_depth);
            return *this;
        }
    ...
    typename binary_search_tree<tkey, tvalue>::node *subtree_root) : current_data(nullptr), traversal_stack(), post_order_stack()
{
    if (subtree_root)
    {
        ...
        while (!traversal_stack.empty())
        {
            auto [current, depth] = traversal_stack.top();
            traversal_stack.pop();
            post_order_stack.push({current, depth});
            if (current->left_subtree)
            {
                traversal_stack.push({current->left_subtree, depth + 1});
            }
        ...
        {
            auto [next_node, next_depth] = post_order_stack.top();
            current_data = next_node->construct_iterator_data(next_depth);
        }
    }
...
typename binary_search_tree<tkey, tvalue>::postfix_iterator &binary_search_tree<tkey, tvalue>::postfix_iterator::operator++()
{
    ...
    if (!post_order_stack.empty())
    {
        auto [next_node, depth] = post_order_stack.top();
        delete current_data;
        current_data = next_node->construct_iterator_data(depth);
    }
    else
    {
        delete current_data;
    ...
    tkey const& key_to_obtain_path_to)
{
    ...
    while (*iter != nullptr)
    {
        auto cmp_res = _tree->_keys_comparer(key_to_obtain_path_to, (*iter)->key);
        if (cmp_res == 0)
        {
            break;
        }
...
binary_search_tree<tkey, tvalue>::insertion_template_method::get_insertion_strategy() noexcept
{
    return insertion_strategy;
}
...
"tvalue binary_search_tree<tkey, tvalue>::disposal_template_method::dispose(tkey const &key)",
    ...
    else
    {
        ...
        else
        {
            ...
            typename binary_search_tree<tkey, tvalue>::node* parent_of_smallest = nullptr;
...
template<
    typename tkey,
    typename tvalue>
typename binary_search_tree<tkey, tvalue>::disposal_of_nonexistent_key_attempt_strategy 
...
binary_search_tree<tkey, tvalue>::binary_search_tree(
    binary_search_tree<tkey, tvalue> &&other) noexcept :
    search_tree<tkey, tvalue>(std::move(other._keys_comparer), std::move(other.get_logger()), std::move(other.get_allocator())),
...
{
    // TODO: сначала поиск ближайшего => lower-bound элемента вместо begin-a
    auto result = std::vector<typename associative_container<tkey, tvalue>::key_value_pair>();
